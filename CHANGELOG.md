...
- 修复供应商配置页和 Codex 增强页在最小桌面窗口下的横向布局与文字竖排问题。
- 恢复并保留「启用 Windows Computer Use Guard」选项，供应商切换时持续维护官方 Computer Use 所需配置。
- 新增青色国风水墨背景，统一深色和浅色主题的背景层与文字对比度。
- 更新 Windows 和 macOS 发布构建所使用的版本号。
