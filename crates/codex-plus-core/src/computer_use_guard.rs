...
///
/// Codex can leave one subprocess per Computer Use session running after a
/// conversation ends. They are recreated lazily for the next session, so
/// periodically removing them prevents unbounded memory growth.
#[cfg(target_os = "macos")]
