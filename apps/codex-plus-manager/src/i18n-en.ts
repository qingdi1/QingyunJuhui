...
export const EN_PLAIN: Record<string, string> = {
  ...
  "Debug 端口": "Debug port",
  "Codex 1.5x Fast": "Codex 1.5x Fast",
  "启用 Codex 1.5x Fast 服务模式按钮；支持模型使用 priority 档位，其他模型按 Standard 发送。":
    "Enable the Codex 1.5x Fast service-tier button; supported models use the priority tier, while other models use Standard.",
  ...
  "正在验证密钥并自动获取可用模型…": "Verifying the key and fetching available models…",
  "未能获取模型，请检查密钥是否有效后重试。密钥不会显示在提示或日志中。": "Could not fetch models. Check that the key is valid and try again. The key will not appear in notices or logs.",
  "Responses API 核验失败，正在自动尝试 Chat Completions…": "Responses API verification failed; trying Chat Completions automatically…",
  "供应商核验未完成，请检查网络后重试。": "Provider verification did not complete. Check the network and try again.",
  "核验通过，正在保存并设为当前供应商…": "Verification passed. Saving and making this the active provider…",
  ...
  "OpenAI 兼容路径由应用自动配置，无需手动填写。": "The OpenAI-compatible path is configured automatically; no manual entry is needed.",
  ...
  "已输入密钥，可直接连接核验。": "A key is entered and ready for verification.",
  "还没有密钥？前往官网登录或注册后获取。": "Do not have a key? Open the official site to sign in or register.",
  "前往官网获取密钥": "Get a key from the official site",
  ...
  "名称排序": "Sort by name",
  "社区主题排序": "Community theme sort",
  "正在加载 DreamSkin 社区…": "Loading DreamSkin Community…",
  "没有匹配的社区主题。": "No matching community themes.",
  "DreamSkin 社区暂时没有可用主题。": "DreamSkin Community has no available themes right now.",
  ...
  "预览": "Preview",
  "搜索市场脚本": "Search market scripts",
  "搜索名称、作者、描述或标签": "Search name, author, description, or tags",
  "脚本市场排版": "Script market layout",
  "板块": "Sections",
  "列表": "List",
  "没有匹配的市场脚本。": "No matching market scripts.",
  "尝试从sub2api获取倍率显示": "Try to fetch the multiplier from sub2api",
  "获取倍率": "Get multiplier",
  "保存前可先尝试从 /v1/sub2api/billing 获取上游倍率。": "Before saving, try fetching the upstream multiplier from /v1/sub2api/billing.",
  "非 Sub2API 供应商不会请求或显示倍率。": "Non-Sub2API providers do not request or display a multiplier.",
  "当前使用中：保留此供应商的 auth 存档，避免 Codex 登录密钥覆盖供应商密钥": "Currently in use: keep this provider's auth archive so the Codex login key does not overwrite the provider key",
  "安全提示：网页链接中的自定义 config.toml 和 auth.json 不会执行；管理工具只会使用上方字段生成受管配置。": "Security notice: custom config.toml and auth.json from web links are never executed; the manager only generates managed configuration from the fields above.",
  "从 DreamSkin.cc 安装主题": "Install theme from DreamSkin.cc",
  "检测到网页一键换肤请求。确认后会从固定社区 API 下载，并在本机重新校验大小、SHA-256、ZIP 清单与 Safe CSS。": "A one-click web theme request was detected. After confirmation, the package is downloaded from the fixed community API and its size, SHA-256, ZIP manifest, and Safe CSS are checked again locally.",
  "主题版本 ID": "Theme version ID",
  "链接不能携带任意下载地址、文件路径或命令；安装后主题会进入“我的主题”，不会自动重启 Codex。": "Links cannot carry arbitrary download URLs, file paths, or commands. After installation, the theme appears in My themes and Codex is not restarted automatically.",
...
export const EN_TEMPLATE: Record<string, string> = {
  ...
  "，补齐 {0} 条侧边栏索引": ", filled {0} sidebar index entrie(s)",
  "已同步到 {0}：修复 {1} 个会话文件，更新 {2} 行数据库索引{3}{4}{5}。":
    "Synced to {0}: repaired {1} session file(s) and updated {2} database index row(s){3}{4}{5}.",
  "已获取 {0} 个模型，正在执行真实请求核验…": "Fetched {0} model(s); verifying a real request…",
  "已生成青云供应商卡，导入 {0} 个模型并设为当前供应商。": "Created a Qingyun provider card, imported {0} model(s), and made it active.",
  "连接成功，已导入 {0} 个模型并设为当前供应商。": "Connected successfully, imported {0} model(s), and made it active.",
  "来自 DreamSkin.cc 的已审核主题，共 {0} 套；安装前仍会在本机再次校验。": "{0} reviewed theme(s) from DreamSkin.cc; each is verified again locally before installation.",
