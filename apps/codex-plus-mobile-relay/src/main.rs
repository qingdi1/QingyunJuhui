...
async fn main() -> anyhow::Result<()> {
    ...
    println!("Qingyun Juhui mobile relay listening on ws://{local_addr}");
    println!(
...
<body>
<div class="app">
    <header class="topbar"><strong class="title">青云聚汇</strong><span id="status" class="status">待连接</span></header>
  <main class="layout">
...
async function ensureAppServer() {
  ...
  await new Promise((resolve, reject) => {
    ...
    setTimeout(() => { if (pending.delete(id)) reject(new Error("app-server 连接超时")); }, 30000);
      ...
      await rpcRaw("initialize", { clientInfo: { name: "Qingyun Juhui Mobile Relay", version: "1.0.0" }, capabilities: { experimentalApi: true } });
}
